...
typedef struct
{
  ...
  BUNION(DIER, unsigned int,                    //              0x0000'0000     TIMx DMA/interrupt enable register
    ...
    _reserved2                          , 17);
/* =================================== CHỖ CẦN SỬA ĐỂ DÙNG: TIM4.PSC.REG = 7999; và TIM4.ARR.REG = 1000;===================================*/
...
/* =================================== PHẦN SỬA ĐỔI: Chuyển các thanh ghi số nguyên thành UNION =================================== */

  // Thanh ghi SR (Status Register) - Offset 0x10
  union {
    unsigned int REG;
    struct {
  ...
  // Thanh ghi EGR (Event Generation Register) - Offset 0x14
  union {
    unsigned int REG;
    struct {
      ...
      unsigned int _reserved1:25;
    } BITS;
  } EGR;

  // Thanh ghi CCMR1 (Capture/Compare Mode Register 1) - Offset 0x18
  union {
    unsigned int REG;
    struct {
      // Output Mode bits (CC1S = 00)
      unsigned int CC1S:2; unsigned int OC1FE:1; unsigned int OC1PE:1;
      unsigned int OC1M:3; unsigned int OC1CE:1;
      unsigned int CC2S:2; unsigned int OC2FE:1; unsigned int OC2PE:1;
      unsigned int OC2M:3; unsigned int OC2CE:1; 
      unsigned int _reserved:16;
    } BITS_OUTPUT;
     struct {
      // Input Mode bits (CC1S != 00)
      unsigned int CC1S:2; unsigned int IC1PSC:2; unsigned int IC1F:4;
      unsigned int CC2S:2; unsigned int IC2PSC:2; unsigned int IC2F:4;
      unsigned int _reserved:16;
    } BITS_INPUT;
  } CCMR1;

  // Thanh ghi CCMR2 (Capture/Compare Mode Register 2) - Offset 0x1C
  union {
    unsigned int REG;
    struct {
      // Output Mode bits (CC3S = 00)
      unsigned int CC3S:2; unsigned int OC3FE:1; unsigned int OC3PE:1;
      unsigned int OC3M:3; unsigned int OC3CE:1;
      unsigned int CC4S:2; unsigned int OC4FE:1; unsigned int OC4PE:1;
      unsigned int OC4M:3; unsigned int OC4CE:1;
      unsigned int _reserved:16;
    } BITS_OUTPUT;
     struct {
      // Input Mode bits (CC3S != 00)
      unsigned int CC3S:2; unsigned int IC3PSC:2; unsigned int IC3F:4;
      unsigned int CC4S:2; unsigned int IC4PSC:2; unsigned int IC4F:4;
      unsigned int _reserved:16;
    } BITS_INPUT;
  } CCMR2;

  // Thanh ghi CCER (Capture/Compare Enable Register) - Offset 0x20
  union {
    unsigned int REG;
    struct {
      unsigned int CC1E:1; unsigned int CC1P:1; unsigned int _res1:2;
      unsigned int CC2E:1; unsigned int CC2P:1; unsigned int _res2:2;
      unsigned int CC3E:1; unsigned int CC3P:1; unsigned int _res3:2;
      unsigned int CC4E:1; unsigned int CC4P:1; unsigned int _res4:2;
      unsigned int _reserved:16;
    } BITS;
  } CCER;

  // Thanh ghi CNT (Counter) - Offset 0x24
  union {
      unsigned int REG;
      struct { unsigned int val:16; unsigned int _reserved:16; } BITS;
  } CNT;

  // Thanh ghi PSC (Prescaler) - Offset 0x28
  union {
      unsigned int REG; // Cho phép dùng TIM4.PSC.REG = 7999;
      struct { unsigned int val:16; unsigned int _reserved:16; } BITS;
  } PSC;

  // Thanh ghi ARR (Auto-Reload Register) - Offset 0x2C
  union {
      unsigned int REG; // Cho phép dùng TIM4.ARR.REG = 1000;
      struct { unsigned int val:16; unsigned int _reserved:16; } BITS;
  } ARR;

  unsigned int _Reserved1; // Offset 0x30
/* =================================== KẾT THÚC PHẦN SỬA ĐỔI: Chuyển các thanh ghi số nguyên thành UNION */
/*// Các thanh ghi CCRx cũng nên chuyển sang Union hoặc giữ nguyên unsigned int tùy bạn
// Ở đây tôi giữ nguyên unsigned int cho đơn giản vì code mẫu chưa dùng tới .REG của CCR
...
/* --- CẬP NHẬT THÊM CHO PWM ---*/
  
  // Thanh ghi CCR1 (Capture/Compare 1) - Offset 0x34
  union {
      unsigned int REG; // Dùng để gán giá trị độ sáng (Duty Cycle)
      struct { unsigned int val:16; unsigned int _reserved:16; } BITS;
  } CCR1;

  // Thanh ghi CCR2 - Offset 0x38
  union {
      unsigned int REG;
      struct { unsigned int val:16; unsigned int _reserved:16; } BITS;
  } CCR2;

  // Thanh ghi CCR3 - Offset 0x3C
  union {
      unsigned int REG;
      struct { unsigned int val:16; unsigned int _reserved:16; } BITS;
  } CCR3;

  // Thanh ghi CCR4 - Offset 0x40
  union {
      unsigned int REG;
      struct { unsigned int val:16; unsigned int _reserved:16; } BITS;
  } CCR4;
